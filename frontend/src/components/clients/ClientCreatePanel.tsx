...
export function ClientCreatePanel({ isOpen, onClose, onCreate }: ClientCreatePanelProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!formData.monthlySalary || formData.monthlySalary <= 0) newErrors.monthlySalary = 'Required'
    if (!formData.loanAmount || formData.loanAmount <= 0) newErrors.loanAmount = 'Required'
    if (!formData.estimatedPropertyValue || formData.estimatedPropertyValue <= 0) newErrors.estimatedPropertyValue = 'Required'
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateField('monthlyLiabilities', Number(e.target.value))}
                placeholder="5000"
                className={INPUT_CLASSES}
              />
            </div>
  ...
  return (
    <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
      <div className="grid grid-cols-3 gap-4">
        ...
        {/* Max Loan */}
        <div>
          <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Max Loan Amount</div>
