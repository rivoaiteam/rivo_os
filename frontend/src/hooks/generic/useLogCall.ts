/**
 * Generic useLogCall Hook
 *
 * Reusable across all entities. Takes entityType as parameter.
 * Logs a call with optimistic update, rollback on error, and query invalidation.
 */

import { useMutation, useQueryClient } from '@tanstack/react-query'
import { entityApi } from '@/api/entity'
import type { EntityType, LogCallInput, EntityWithActivity, CallLog } from './types'
import { ENTITY_QUERY_KEYS } from './types'

export function useLogCall(entityType: EntityType) {
  ...
  return useMutation({
    mutationFn: ({ entityId, outcome, notes }: LogCallInput) =>
      entityApi.logCall(entityType, entityId, outcome, notes),

    onMutate: async ({ entityId, outcome, notes }) => {
      ...
      if (previous) {
        ...
        queryClient.setQueryData(detailKey, {
          ...previous,
          callLogs: [newCallLog, ...(previous.callLogs || [])],
        })
      }
