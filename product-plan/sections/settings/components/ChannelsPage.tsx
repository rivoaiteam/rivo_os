...
interface ChannelsPageProps {
  ...
  onChangeStatus?: (id: string, status: ChannelStatus) => void
}
...
export function ChannelsPage({
  ...
  const handleEdit = (id: string) => {
    if (editName.trim()) {
      onEditChannel?.(id, editName.trim())
      setEditingId(null)
      setEditName('')
    }
  }
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Manage channels for client acquisition
            </p>
          </div>
          {!isAdding && (
            <button
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 min-h-[400px]">
          ...
          {channels.length === 0 && !isAdding ? (
            <EmptyState message="No channels yet. Add your first channel." />
          ) : (
            <div className="divide-y divide-slate-200 dark:divide-slate-700">
              {channels.map((channel) => {
                const currentOption = statusOptions.find((opt) => opt.value === channel.status)
                return (
                  <div
                    key={channel.id}
                    ...
                    {editingId === channel.id ? (
                      <>
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') handleEdit(channel.id)
                      ...
                      <>
                        ...
                        <StatusBadge
                          ...
                          isOpen={statusDropdownId === channel.id}
                          onToggle={() => setStatusDropdownId(statusDropdownId === channel.id ? null : channel.id)}
                          onClose={() => setStatusDropdownId(null)}
                          onChange={(status) => {
                            onChangeStatus?.(channel.id, status)
